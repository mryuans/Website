...
@RECENT_UPDATES
QuickCheck
`,n:.243}}},{i:19,$:{0:{v:"Misc",n:1}}},{i:20,$:{0:{v:"CustomGrammer",n:1},1:{v:`I’d like to test sth
...
`,n:.129}}},{i:21,$:{0:{v:"CS related",n:.707}}},{i:22,$:{0:{v:"lec5",n:1},1:{v:`在不同编辑器中尝试调用openssl中的md5函数：
...
`,n:.218}}},{i:23,$:{0:{v:"lec4",n:1},1:{v:`ring settings
...
`,n:.089}}},{i:24,$:{0:{v:"lec3",n:1},1:{v:`这节课主要学习了多表加密的enigma加密机。
Machine Structure
...
`,n:.051}}},{i:25,$:{0:{v:"lec2",n:1}}},{i:26,$:{0:{v:"lec1",n:1}}},{i:27,$:{0:{v:"Cryptography",n:1}}}]},t=[{title:"markdown-it",content:`本文将简略介绍本框架使用的markdown解析器，markdown-it与本框架中对于markdown-it的包装和设定以及后续给出的插件包装组件
Github of markdown-it
Parsing and Rendering
markdown-it在整个解析阶段分为两个部分，第一个是parse，获得类似json的包含token的文件，第二个是Render，将第一部分获得的内容渲染成html
Parse 解析
...
另一种则是使用vue的组件，这种类型一般需要相对复杂的ui交互或者排版，这种情况会在tags.ts里面使用驼峰命名规范来改写成vue能读懂的格式，例如<fold> => <Fold>，<dot-pattern> => <DotPattern>
然后这些完整的内容会交由别的接口进行进一步的加工
`,link:"/predoc/markdown_analysis/markdown-it",is_index:!1,created:"2026-04-03T03:51:05.749Z"},{title:"markdown_analysis",content:"",link:"/predoc/markdown_analysis",is_index:!0,created:"2026-04-03T03:51:05.679Z"},{title:"Markdown 词法分析 (Tokens)",content:` Markdown 词法与环境结构分析这些接口与属性共同定义在处于中央调度地位的 src/global.ts，为 Markdown-it 这个超轻量的核心转换引擎进行底层干预提供了非常强有力的定制级强结构挂载依据。它们使得诸如数学公式段落渲染、勾选控件识别及替换以及各类混合语言的代码块排版任务，在剥除传统状态的同时可以被彻底解耦进行微操管理。
 数据分析提取结构承担了系统区分拦截被 YAML 分界符 --- 所包围阻绝断开的头部描述块字典记录（front_matter），以及那些并未曾接受下流底层解析动作染洗处理过的完全纯正的文章主体数据载荷（markdown 的本源文本内容片段保留）。并在适当的生命周期抛出这些快照供各方提取查阅。
由于我们需要基于鼠标快速的跳转导航系统完成页面快速锚点，系统在剥除全套段落与纯文字前专门设置了用于截获拦截由 # 堆砌的所有的级联标题数据抓取探针模块并产出对应的高维标识体系记录栈：
基础脱水骨架 (MarkdownHeader): 主要包含纯长字符串的章节呈现标题文字主体内容、用表示深度级别从属层级的 level 数字定义以及在执行跳转后于页面顶端地址出现的经过清洗的 hash 重定向锚点。
...
`,link:"/predoc/markdown_analysis/MarkdownInterfaces",is_index:!1,created:"2026-04-03T03:51:05.611Z"},{title:"predoc",content:"",link:"/predoc",is_index:!0,created:"2026-04-03T03:51:05.544Z"},{title:"Entry Main",content:`本文将着重介绍插件中的enrty类
##类属性
`,link:"/predoc/entry/temp",is_index:!1,created:"2026-04-03T03:51:05.478Z"},{title:"Entry class",content:"",link:"/predoc/entry",is_index:!0,created:"2026-04-03T03:51:05.413Z"},{title:"Entry 关联的类型定义",content:` Entry 关联的类型定义解析单篇 Markdown 的逻辑里大量使用了类型重命名，它们都在 src/entry/index.ts 下进行管理，严格规范了系统内外接口通讯。
**URL = \\/\${string}\`:** 文件的虚拟映射路由路径（如 /cs/index\`）。
...
`,link:"/predoc/entry/EntryTypes",is_index:!1,created:"2026-04-03T03:51:05.347Z"},{title:"Entry 核心类详解",content:` Entry 核心类详解在框架底层，每一个独立的 Markdown 文件都会被抽象为实例化、对象化的 Entry 类。Entry 承担了数据容器与懒加载编译的双重责任，是系统预处理与增量更新中最重要的数据结构。
Entry 定义于 src/entry/index.ts 中，它的设计具有高度的"延迟加载"特性。如果没有组件试图读取如 html 或 text，其内部的解构与转换计算便不会被执行。
 核心属性 (Properties)pathname: Pathname: Markdown 文件的相对路径（例如 "docs/cs/ads/avl-tree.md"）。
...
`,link:"/predoc/entry/EntryClass",is_index:!1,created:"2026-04-03T03:51:05.281Z"},{title:"temp",content:"",link:"/predoc/component/temp",is_index:!1,created:"2026-04-03T03:51:05.211Z"},{title:"component",content:"",link:"/predoc/component",is_index:!0,created:"2026-04-03T03:51:05.143Z"},{title:"Vite 插件配置选项 (Plugin Options)",content:` Vite 插件选项配置 (Option Interface)在项目的根节点之下的构建控制中心文件 vite.config.js 内部或者用于搭建服务端热更服务的单向进程加载池之中，我们将通过暴露的全局设定选项将它注入并在最内层的暴露实例化类中启动这一全套服务：
// 定义在 vite-plugin-vue-xecades-note/src/global.ts
export interface NotePluginOptions {
    /** 注入存放所有的自定义 Markdown UI 组件目录相对路径 */
    componentDir: string;
...
此非必须赋值的强行性命名改写支持让所有运行在这个引擎下的多独立实例存在着独立平行双开机制安全隔离体系系统的基础。
假若存在有必须要在此套程序底层上运转有很大差别的其他类型博客文件结构的话，如果能给每个运行域提供赋以此截然不同的系统唯一编号或者是新全称标记名字即可！最终在内部缓存生成的管理标识管理操作和处理依赖包捆绑装配引用路径绑定识别处理全阶段里所有的动作它皆将会各自完全地相互无冲突化地区别分别独立地平铺工作下去以此成功杜绝各类运行组件模块产生同名以及重写造成相互干扰所形成的难以觉察到或复现缓存交叉以及触发渲染更新热崩溃等问题现象出现！
`,link:"/predoc/component/PluginOptions",is_index:!1,created:"2026-04-03T03:51:05.076Z"},{title:"cache",content:"",link:"/predoc/cache/temp",is_index:!1,created:"2026-04-03T03:51:05.009Z"},{title:"cache system",content:"",link:"/predoc/cache",is_index:!0,created:"2026-04-03T03:51:04.942Z"},{title:"Cache 全局路由结构",content:` Cache 全局接口结构主要定义和存储在 src/global.ts 中，控制全局导航树、搜索检索格式、Vue 路由记录元信息的结构规范。
系统读取完根目录的设置后会解析出一层层的树形导航 Config.nav：
title: string: 导航块头部的大写名称。
...
此数据结构在最外层继承了官方原生的 RouteRecordSingleView 借口后，合并生成为 CachedRouteRecord 字典强行包裹这套扩展的 RouteMeta。
`,link:"/predoc/cache/GlobalInterfaces",is_index:!1,created:"2026-04-03T03:51:04.876Z"},{title:"配置约束与约定",content:` 配置约束与设定常规 (Convention)该系列配置约束集中声明于 src/convention.ts文件内，划定了从主文件目录树开始延伸而出至用户实际在书写博客和规划侧边栏时的规约底线。
文章顶部通过 --- 包含的一块 YAML 配置块，用以声明影响本篇阅读的基础配置及核心识别参数。
title: string: 唯一识别名或正式的大标题（关联全站搜索引擎的首要抓取特征量）。
...
`,link:"/predoc/cache/ConventionInterfaces",is_index:!1,created:"2026-04-03T03:51:04.803Z"},{title:"websiteGuide",content:`注意：框架仍处于开发早期，存在很多尚未解决以及未知的bug，请慎重使用此框架。
快速上手
本项目使用node.js框架下的vue3-vite框架组合，需要安装node.js与npm进行项目网站管理，下载参考网站框架可以使用如下命令：
$git clone --recursive http://github.com/Xecades/Note.git
...
`,link:"/predoc/UserGuide/websiteGuide",is_index:!1,created:"2026-04-03T03:51:04.730Z"},{title:"MainStructure",content:`本项目是vue3 vite架构搭建，主要的启动路线符合vite服务启动的基本逻辑：
    main.js/ts 启动，作为主入口导入一系列库与字典，锚定vue文件 ->
    App.vue 启动第一层外部 ->
    index.html 接收App.vue嵌入框架
...
最后就是footer标签定义了每一个页面的页脚信息
那么以上就是该项目的主要页面框架，从上面的介绍中可以了解到所有能看到的元素的出处，需要进一步了解只需要了解标签中的代码实现即可
`,link:"/predoc/UserGuide/mainStructure",is_index:!1,created:"2026-04-03T03:51:04.658Z"},{title:"UserGuide",content:"",link:"/predoc/UserGuide",is_index:!0,created:"2026-04-03T03:51:04.586Z"},{title:"主页",content:`本网站施工中，目前主要用于记录有关Xecades/Note笔记网站框架的学习记录与个人文档 如果对项目感兴趣的朋友欢迎访问项目仓库
Xecades’ Note Github plugin
  ID: mryuans   浙江大学 能源与环境系统工程 24级    CTFer 主攻web
  页面总数：@PAGE_COUNT
  总词数：@WORD_COUNT
...
@RECENT_UPDATES
QuickCheck
`,link:"/",is_index:!0,created:"2026-04-03T03:51:04.516Z"},{title:"Misc",content:"",link:"/Misc",is_index:!0,created:"2026-04-03T03:51:04.445Z"},{title:"CustomGrammer",content:`I’d like to test sth
I want to try some custom grammer
How about the fold
Let’s Gooooo…
this is a quote for nobody.
...
`,link:"/Misc/CustomGrammer",is_index:!1,created:"2026-04-03T03:51:04.373Z"},{title:"CS related",content:"",link:"/CS_related",is_index:!0,created:"2026-04-03T03:51:04.301Z"},{title:"lec5",content:`在不同编辑器中尝试调用openssl中的md5函数：
Code pieces 1:#include <openssl/md5.h>
#pragma comment(lib, "libeay32.lib")
#pragma comment(lib, "ssleay32.lib")
main()
...
`,link:"/CS_related/Cryptography/lec5",is_index:!1,created:"2026-04-03T03:51:04.229Z"},{title:"lec4",content:`ring settings
回顾上一周的内容,我们知道选择engima的加密模式存在两种抉择点：
齿轮选择与排列
...
Double spinning
基于上面的条件，实际上运行的时候存在一种特殊情况，也就是当最右侧齿轮带动中间齿轮旋转一次之后，中间齿轮刚好位于下一次可以带动最左侧齿轮的情况，此时再输入一次，这时会导致三个齿轮同时旋转：  两次旋转导致了中间齿轮连续两次旋转，也就是Double spinning.
`,link:"/CS_related/Cryptography/lec4",is_index:!1,created:"2026-04-03T03:51:04.158Z"},{title:"lec3",content:`这节课主要学习了多表加密的enigma加密机。
Machine Structure
Enigma加密机的主要需要关注的结构是五个具有信息映射功能的齿轮，以及一个插线板
Enigma提供的加密信息密度主要来源于以下几个结构：
Message Key
...
假设最右侧的齿轮的旋转点在R与S之间，那么我们就能看到，并非是旋转26次一定就会造成一次进位，尤其是在每一个齿轮的第一次旋转之前，这是依赖于这个齿轮的初始状态的
这个初始状态就被称为Message Key.
`,link:"/CS_related/Cryptography/lec3",is_index:!1,created:"2026-04-03T03:51:04.086Z"},{title:"lec2",content:"",link:"/CS_related/Cryptography/lec2",is_index:!1,created:"2026-04-03T03:51:04.016Z"},{title:"lec1",content:"",link:"/CS_related/Cryptography/lec1",is_index:!1,created:"2026-04-03T03:51:03.949Z"},{title:"Cryptography",content:"",link:"/CS_related/Cryptography",is_index:!0,created:"2026-04-03T03:51:03.884Z"}],r={keys:["title","content"],includeMatches:!0,ignoreLocation:!0,threshold:.4},i=e.parseIndex(o),a=new e(t,r,i),d=n=>n?a.search(n):t;export{d as default};
