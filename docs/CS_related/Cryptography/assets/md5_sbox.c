...
//#1_begin--------------
void sboxSwapper(unsigned char *d, unsigned char *k);

int calculatePos(int col, int row) {
    return (row * 16 + col);
}
...
void sboxSwapper(unsigned char *d, unsigned char *k) {
    ...
    memcpy(dl, d, 3);
    memcpy(dh, d + 3, 3);
    dproccess[0] = (dl[0] >> 2) & 0x3F;
    dproccess[1] = (dl[0] << 4 | dl[1] >> 2) & 0x3F;
    dproccess[2] = (dl[1] << 2 | dl[2] >> 6) & 0x3F;
    dproccess[3] = dl[2] & 0x3F;
    dproccess[4] = (dh[0] >> 2) & 0x3F;
    dproccess[5] = (dh[0] << 4 | dh[1] >> 2) & 0x3F;
    dproccess[6] = (dh[1] << 2 | dh[2] >> 6) & 0x3F;
    dproccess[7] = dh[2] & 0x3F;
    for (int i = 0; i <= 7; i ++) {
        row = dproccess[i] & 0x01 + (dproccess[i] >> 4) & 0x02;
        col = (dproccess[i] >> 1) & 0x10;
        output[i] = calculatePos(col, row);
        if (i % 2 == 1) {
            k[i / 2 - 1] = (output[i] << 4) | output[i - 1];
        }
    }
}
